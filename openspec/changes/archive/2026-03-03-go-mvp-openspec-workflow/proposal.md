...
- Keep one canonical state taxonomy (`S0..S8`, `DONE`), but split **state storage ownership**:
  ...
  - `spln init` SHALL pre-create evidence roots `.spln/evidence/skills/`, `.spln/evidence/tasks/`, `.spln/evidence/runs/`
...
- Keep routing as two-dimension evaluation with:
  ...
  - executable intake with non-empty `blocking_unknowns[]` stays in speclane and routes to `L3` discovery path in auto mode
...
- Define guardrail high-risk check catalog for `G_ship` with deterministic missing/failing reason codes (MVP baseline uses one mandatory domain baseline check ID per domain)
...
- Define governed required artifact bundle for L2/L3:
  ...
  - `L3` adds `explore.md`
  - ship-stage required approval set excludes `explore.md` (scope-stage only via `S2/S3 + G_scope`)
...
- Add explicit pivot command contract:
  - add `spln pivot`
  - `spln pivot` supports explicit intent kind (`reroute|rescope`, default `reroute`)
  - pivot execution order is analyze-first: `S1_ANALYZE` refresh -> `G_pivot` evaluation -> apply reroute/rescope
  - `rescope` is valid only from governed `S6_RUN_WAVES`; `S7/S8` `rescope` requests are deterministic precondition rejections
...
- Extend evidence contract with:
  - lean core required fields (`run_summary_version`, `session_id`, skill identity, verdict payload, timestamp)
  - conditional traceability fields (`input_hash` required only for run-summary-bound `S6/S7/S8`; optional in pre-run states; optional `input_scope` for traceability/hash explainability)
  - optional denormalized `mitigation_target` (when present MUST match skill mapping; when absent derive by `skill_name`)
  - optional actor metadata (`actor_id`, `role`) for audit enrichment without hard readiness coupling
...
- Define frozen run-summary persistence contract:
  - immutable files at `.spln/evidence/runs/<request_id>/rv<run_summary_version>.json`
  - lane-state pointer `latest_frozen_run_summary_version` for review/verify readers
- Define deterministic `non_spln` CLI rejection mapping:
...
- `skill-contracts`: state-bound skills with lean evidence contract (`session_id`, conditional `input_hash`, optional mitigation/actor metadata)
