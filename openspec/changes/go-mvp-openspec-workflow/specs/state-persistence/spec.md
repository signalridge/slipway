...
### Requirement: YAML-Only Persistence (No DB)
MVP SHALL use filesystem YAML persistence only. No runtime database SHALL be required.
...
Governance evidence records SHALL persist:
- `run_summary_version`
- `session_id`
- `input_hash`
- `mitigation_target`
...
These fields SHALL be required for evidence considered valid by review/verify logic.
...
- `actor_id` SHALL be stable actor identity for audit correlation
- `role` SHALL be one of `implementer|reviewer|operator`

`input_hash` persistence contract:
- SHALL be lowercase hex SHA-256 value
- SHALL be computed from canonical JSON payload defined by skill-contracts (`request_id`, `state`, `run_summary_version`, normalized `input_scope`, relevant fingerprints/identifiers)
...
#### Scenario: Missing mitigation target evidence rejected
- **WHEN** governance evidence lacks `mitigation_target`
- **THEN** evidence validation SHALL fail for readiness checks
