...
- deterministic consumer thresholds SHALL be applied over `intake_assessment`:
  - `executable` if `is_executable=true`, `confidence>=0.65`, and concrete delta/targets exist
  - `non_spln` for clear advisory/question (`confidence>=0.75`, `is_executable=false`)
  - otherwise `non_spln` with clarification remediation (`non_spln:clarification_required`)
...
## Non-Normative Reference: Source Mapping

This section is informative only and does not define runtime pass/fail behavior.
It exists to clarify where design influence comes from when reading this workflow.

| State | Primary Reference | Intended Influence |
|---|---|---|
| `S0_INTAKE` | SpecLane local | intake and route setup |
| `S1_ANALYZE` | SpecLane local + Superpowers | uncertainty reduction and minimal analysis |
| `S2_DISCOVER` | Superpowers | discovery-first exploration |
| `S3_SCOPE_CONFIRMATION` | Superpowers + SpecLane local | scope confirmation and worktree readiness |
| `S4_SPEC_BUNDLE` | OpenSpec | governed artifact bundle semantics |
| `S5_PLAN_AUDIT` | SpecLane local | run-readiness governance |
| `S6_RUN_WAVES` | GSD | dependency-layered execution |
| `S7_REVIEW` | OpenSpec + SpecLane local | layered review loop |
| `S8_VERIFY` | OpenSpec + GSD | verify against goals/blockers and run conditional closeout refresh in governed lane |
| `DONE` | OpenSpec + SpecLane local | completion and archive semantics |

Conflict handling for implementation remains local-first:
SpecLane contract > reference patterns.
