...
| precondition_blocked | `3` | no active request, ambiguous active context, lock timeout |
...
- deterministic consumer thresholds SHALL be applied:
  - executable when `is_executable=true`, `confidence>=0.65`, and concrete target/delta exists
  - `non_spln` advisory when `intent_type in {advisory, question}` and `confidence>=0.75`
  - otherwise `non_spln` with clarification remediation
...
`spln pivot` SHALL trigger re-analysis and reroute.
...
`spln pivot` SHALL first set workflow state to `S1_ANALYZE` and execute analyze before applying any reroute outcome.
...
- **THEN** command flow SHALL transition to `S1_ANALYZE` before any level or lane update
...
#### Scenario: Pivot does not require reason flag
- **WHEN** `spln pivot` is run with default arguments
- **THEN** command SHALL continue to evaluate active-context and state-boundary preconditions without argument-validation failure
