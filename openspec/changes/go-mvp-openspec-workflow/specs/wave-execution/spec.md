...
- `pivot` from `S6` SHALL always transition to `S1_ANALYZE` first
- if analyze/reroute changes effective level, workflow follows reroute path from `S1`
- if effective level remains governed and operator intent is rescope without reroute:
