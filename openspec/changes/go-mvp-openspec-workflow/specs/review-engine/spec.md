...
- `IR2` is optional in MVP and only runs when explicitly requested by policy or operator
- `IR3` requirement is independent from `IR2`; guardrail-sensitive scope MAY run `IR1 -> IR3` directly when IR2 is not requested
- additional artifact-layer escalation beyond minimum set MAY run but SHALL NOT be required in non-guardrail MVP baseline
...
- review reads the latest completed run summary snapshot
...
- guardrail-sensitive governed scope: changed governed artifacts run full `R0..R3`
...
| guardrail-sensitive governed scope | changed governed artifacts (`proposal/spec/design/tasks/assurance`) | `R0..R3` | guardrail precedence applies |
...
- when guardrail rules and default matrix rows conflict, guardrail-required R3 behavior SHALL take precedence
