...
1. classify as `executable` when all are true:
   - `is_executable=true`
   - `confidence >= 0.65`
   - (`change_targets[]` is non-empty OR `intended_delta` is non-empty)
2. classify as `non_spln` when all are true:
   - `intent_type in {advisory, question}`
   - `is_executable=false`
   - `confidence >= 0.75`
3. otherwise (`mixed|unclear`, low confidence, or `blocking_unknowns[]` non-empty):
...
3. `new_project` or `major_refactor` => `L3`
4. `ambiguity >= 3` and `control_score >= 8` => `L3`
5. guardrail exists => `L3`
6. `control_score >= 8` => `L2`
7. otherwise => `L1`
...
- `intake_assessment` (at minimum: `intent_type`, `is_executable`, `confidence`, `acceptance_anchor`, `blocking_unknowns[]`)
- `scores` (raw)
...
- after explicit pivot invocation, `G_pivot` is evaluated before reroute/rescope is applied

After pivot is approved, workflow SHALL transition to `S1_ANALYZE` before any level update or reroute decision.
...
- **THEN** workflow SHALL first transition to `S1_ANALYZE` and only then persist reroute outputs
