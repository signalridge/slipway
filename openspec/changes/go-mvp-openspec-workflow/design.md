...
**Decision:** Use YAML files + lock + atomic write.

- YAML serialization (`yaml.v3`)
...
**Decision:** All governance evidence records must include session and input traceability.
...
- field is denormalized for audit readability/indexing
- value MUST match the registered skill-to-mitigation mapping for `skill_name`
- mismatch invalidates governance readiness

Actor identity contract:
- `actor_id` is stable actor identity for audit correlation across evidence/events
- `role` captures execution role (`implementer|reviewer|operator`) used by reviewer-independence checks
- both fields are required for forensic traceability and SHALL NOT be omitted
...
These fields support reviewer independence and stale-evidence detection.
...
type EvidenceRecord struct {
    ...
    InputHash    string    `json:"input_hash"` // canonical-input SHA-256 lowercase hex
    MitigationTarget string `json:"mitigation_target"`
...
type TaskRun struct {
    ...
    EvidenceRef string   `yaml:"evidence_ref,omitempty"` // deterministic task evidence path: .spln/evidence/tasks/<request_id>/rv<run_summary_version>/<task_id>.json[--n]
...
Mitigation: enforce `session_id + input_hash` and stale-evidence checks in review/verify.
