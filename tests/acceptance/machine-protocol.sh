...
expect_error 2 invalid_budget resume-ad-hoc "$BIN" _machine resume "$BUDGET_RUN" --root "$BUDGET_REPO" --budget 0
...
SOURCE_PARENT_REQUIREMENTS=$(json_get "$SOURCE_START" source.requirements_revision)
SOURCE_MATERIAL="$TMP_ROOT/source-material.json"
"$BIN" _machine material --root "$SOURCE_REPO" --run "$SOURCE_RUN" --action "$(json_get "$SOURCE_START" action_id)" --section requirements > "$SOURCE_MATERIAL"
...
"$BIN" _machine material --root "$SOURCE_REPO" --run "$SOURCE_RUN" --action "$(json_get "$SOURCE_ADOPTED" action_id)" --section requirements > "$SOURCE_ADOPTED_MATERIAL"
