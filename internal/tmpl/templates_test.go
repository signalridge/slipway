...
func TestCapabilityPromptsEncodeAcceptanceBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "skills/run/SKILL.md", contains: []string{"explicitly asks", "marker-valid Run", "structured `next.variants`", "complete pinned", "`grill-me` design-tree discipline", "shared understanding", "Strict Outcome shape", "`skipped` is emitted only by the CLI", "Review never needs input", "Every Action may be skipped", "automatic Action queue is empty", "`gh >= 2.94.0`", "official REST fallback", "redirects/transfers only within `github.com`", "accepted five Requirements sections", "Redact recognized credentials"}},
		...
		{path: "skills/implement/SKILL.md", contains: []string{"explicitly invokes", "pinned Requirements", "activities` (which may be empty)", "actual positive attempt count", "exact command", "exit code", "Never list an activity that did not run", "shell exit 127", "scope SHA-256", "directly outside a Run"}},
		{path: "skills/review/SKILL.md", contains: []string{"explicitly invokes", "always read-only", "Intent", "Quality", "start-to-current difference is only an observation", "findings_reported", "leave `suggested_actions` empty", "Do not modify files", "automatic repair or re-review loop"}},
	}
