...
func TestCapabilityPromptsEncodeAcceptanceBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "skills/run/SKILL.md", contains: []string{"explicitly asks", "marker-valid Run", "structured `next.variants`", "ordered chapter catalog", "`grill-me` design-tree discipline", "shared understanding", "Strict Outcome shape", "`action_kind` is mandatory", "`skipped` is emitted only by the CLI", "Review never needs input", "Every Action may be skipped", "overrides and discards that pending suggestion", "later revision after a prior Review", "automatic Action queue is empty", "`gh >= 2.94.0`", "official REST fallback", "redirects/transfers only within `github.com`", "accepted chapter materials", "Redact recognized credentials"}},
