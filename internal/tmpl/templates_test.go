...
func TestCapabilityPromptsEncodeAcceptanceBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{path: "skills/workflow/SKILL.md", contains: []string{"explicitly asks to run the workflow", "stateless only in the Slipway sense", "default idea-to-draft path is read-only", "self-contained and must work when no Matt Pocock skill is installed", "Never invoke a user-only front door", "`code-review` even when it is model-reachable", "one genuine decision at a time", "Model-invocable primitives are optional accelerators", "model-invocable `/grilling` primitive", "run the `/grilling` skill", "shared-understanding confirmation rules", "Artifact-producing primitives", "`/domain-modeling` writes glossary or ADR material", "`/research` writes a Markdown report", "`/prototype` writes throwaway code", "Do not install or enable a primitive", "not a durable wayfinding state machine", "`kind:research` Change", "For an Objective, instead produce its distinct planning shape", "Matt's `to-tickets -> implement -> code-review` path stops at this boundary", "not an approved publication plan", "Only a marker-valid `change/v2` Issue can be an executable Issue source", "never hand a bare Issue number to the CLI", "two deliberate authorization boundaries", "ad-hoc `slipway-run` on the sharpened goal", "`paused`, `stopped`, or `ended`", "automatic repair or re-review loop", "`budget_exhausted` pause is normal", "`max(initial_budget, 3)`", "Certify nothing", "never an implicit subroutine"}},
	}
