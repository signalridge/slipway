...
- Before spawning a `parallel: true` wave, run a target-overlap preflight over the current wave's `target_files`. If two tasks overlap by exact path, path alias, parent/child scope, case-insensitive match, or glob scope, record `dispatch_blocker:wave=<wave_index>:target_overlap` and stop for explicit operator direction before continuing; target overlap is not a degraded-sequential fallback path.
...
- A target-overlap preflight failure records `dispatch_blocker:wave=<wave_index>:target_overlap` and stops for operator replanning; do not resolve it by merely marking the wave `degraded_sequential`.
