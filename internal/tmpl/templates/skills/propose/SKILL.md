...
A managed Change starts with the exact first non-empty line `<!-- slipway-level: change/v2 -->`, has a `[Change]` title, exactly one `level:change` label, exactly one `kind:*` label, and one strict Issue-body manifest. Publish Outcome, Requirements, Acceptance examples, Constraints, and Non-goals as independently addressable chapter comments; each starts with its exact `<!-- slipway-section:v1 key=KEY -->` marker. The manifest's ordered entries bind each key/role/title to the comment node ID, database ID hint, and exact body digest. It must be self-contained; unreferenced discussion and a parent Objective are not runtime inheritance. `ready-for-agent` is optional navigation only.
