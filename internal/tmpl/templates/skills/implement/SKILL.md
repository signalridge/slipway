...
When invoked directly outside a Run, there is no Action ID, source revision, run-start attribution, or Outcome submission. Treat the explicit request as ordinary implementation authorization, use at most three repair attempts unless the user specifies another limit, and return the same factual information in human-readable form without inventing protocol identifiers.
