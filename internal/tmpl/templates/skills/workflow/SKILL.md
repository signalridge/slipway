...
| Active issue-backed Run whose user requests a Change amendment | Explain that chat prose cannot revise pinned Requirements, that the amendment needs a stop through public `slipway stop` before an explicit resume with its refresh/use-pinned/candidate choice, and report the currently pinned revision; never run `slipway stop` from here | `slipway-run` |
| Active Run | Report the exact current Action and its submit/skip variants; state that stop uses public `slipway stop`, that take-over and reorder stop first and hand control back, and that this capability runs none of them | `slipway-run` |
...
When a draft is needed, settle the testing seams, slice granularity, and blocking edges that would otherwise prevent a self-contained downstream draft.

The optional `/grilling` primitive is an accelerator, never a dependency or gate, and this discipline governs it rather than the reverse. Some published revisions of `/grilling` ask a whole round of questions at once; if the installed one does, state that this invocation overrides it to one decision per response and keep asking that way. If it cannot be held to one decision per response, do not use it: apply this discipline directly, which is also what you do when `/grilling` is absent. If the user asks to wrap up at any point, stop the interview immediately; summarize settled decisions and open questions without asking another confirmation or routing. Otherwise, if interview answers add to or change the execution understanding embodied in the draft, summarize the current shared understanding and make one explicit confirmation the next single question. If there was no interview or the understanding did not change, do not make a duplicate confirmation. This confirmation grants no publication, decomposition, implementation, or Run authority; the initial explicit workflow invocation already authorized drafting.
