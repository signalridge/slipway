...
func List(root string) ([]HostStatus, error) {
	...
	for _, host := range hosts {
		...
		// loadManifest accepts only the current manifest version.
		...
		if found {
			...
			if inspection.StaleClaims > 0 {
				status.Warning = fmt.Sprintf(
					"ownership manifest contains %d file claim(s) whose bytes are not generated by this version; refresh or uninstall will preserve those files and withdraw the stale claims",
...
func doctorWithInspector(
	...
	for _, host := range hosts {
		...
		case inspection.StaleClaims > 0:
			report.Checks = append(report.Checks, DoctorCheck{
				Code:   "adapter_refresh_required",
				Status: "warning",
				HostID: host.ID,
				Name:   "adapter",
				Detail: fmt.Sprintf(
					"%d ownership claim(s) do not match bytes generated by this version; refresh or uninstall preserves those files and withdraws the stale claims; inspect and remove preserved files manually before refresh if regeneration is desired",
					inspection.StaleClaims,
				),
			})
	...
	if found {
		...
		for _, record := range manifest.Files {
			...
			if currentClaims[record.Path] != record.SHA256 {
				...
				plan.warnings = append(plan.warnings, staleOwnershipClaimWarning(host, record.Path))
				continue
			}
			classification, err := classifyFileWithFilesystem(filesystem, absolute, record.SHA256)
			if err != nil {
...
func loadManifestWithFilesystem(filesystem ownershipFilesystem, root string, host Host) (ownershipManifest, bool, error) {
	...
	// The manifest is untrusted repository JSON. Every claim must stay inside
	// the current generated path surface. Hash equality is intentionally checked
	// later, at each mutation decision: a same-version manifest can outlive a
	// template byte change, but its old self-reported digest is indistinguishable
	// from a forged digest and therefore may only cause per-file preservation.
	currentClaims, err := currentGeneratedClaims(host)
	if err != nil {
		return ownershipManifest{}, false, err
	}
	for _, record := range manifest.Files {
		if _, allowed := currentClaims[record.Path]; !allowed {
			return ownershipManifest{}, false, fmt.Errorf("ownership manifest for %s claims path %q that is not a current managed path", host.ID, record.Path)
		}
	}
...
// currentGeneratedClaims returns the current path allowlist and exact generated
// digests. Path membership validates manifest shape; only digest equality may
// authorize overwriting or deleting an existing file.
...
func staleOwnershipClaimWarning(host Host, relative string) string {
	return fmt.Sprintf(
		"ownership manifest for %s claims %q with a digest that does not match bytes generated by this version; the file is preserved and the stale claim is withdrawn; inspect it, then remove the preserved file manually and rerun slipway install --refresh if regeneration is desired",
		host.ID,
		relative,
	)
}
