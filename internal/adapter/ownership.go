...
func loadManifest(root string, host Host) (ownershipManifest, bool, error) {
	...
	for _, record := range manifest.Files {
		allowed, err := currentClaimAllowed(host, record.Path, record.SHA256)
		if err != nil {
			return ownershipManifest{}, false, err
		}
		if !allowed {
...
func currentClaimAllowed(host Host, relative, claimedSHA256 string) (bool, error) {
	...
	for _, file := range files {
		if file.Relative == relative {
			// Anchor each claim to the current binary's actual generated
			// content, not just to a path that the current surface would
			// produce. A manifest is untrusted user-supplied JSON; a path
			// match alone lets a forged manifest claim a user file whose
			// content happens to match the manifest's self-reported hash.
			// Requiring the claimed hash to equal the freshly generated
			// template content ties deletion authorization to genuine
			// Slipway provenance without introducing any hard gate.
			return claimedSHA256 == hashBytes(file.Data), nil
		}
	}
	return false, nil
}
