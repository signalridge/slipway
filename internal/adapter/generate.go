...
func generateHostFiles(host Host) ([]generatedFile, error) {
	...
	for _, capability := range capabilityNames {
		...
		case "copilot_agent":
			...
			files = append(files, generatedFile{
				Relative: filepath.ToSlash(filepath.Join(".github/copilot/agents", capability+".agent.md")),
				Data:     []byte(content),
			})
		case "kilo_command":
			files = append(files,
	...
	files = append(files, generatedFile{
		Relative: filepath.ToSlash(filepath.Join(referenceRoot(host), "slipway-clarify", "references", "decision-interview.md")),
		Data:     []byte(reference),
	})
	return files, nil
}
