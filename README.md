...
`tasks_plan_hash` / content digests rather than file mtimes or legacy
`evidence_input_hash`; legacy hash-only summaries remain parseable only so
diagnostics can mark them stale. `wave-plan.yaml` still records `generated_at`
for display and audit, but freshness does not compare against that timestamp.
...
- `artifacts/changes/**/verification/evidence-digests.yaml` is engine-owned local freshness state. Slipway writes it when accepted skill evidence is consumed and uses it to detect which certified input changed. Each entry records the accepted verification verdict timestamp so a later re-run verdict can replace a stale digest during mutating advancement; the timestamp is not used as the content freshness signal. Diff-class review entries certify the current working diff (`git diff HEAD` plus non-ignored untracked reviewable files, excluding Slipway governed/runtime artifacts under `artifacts/changes/**`), so committing reviewed changes before finalization can transiently stale read-only projections until the next mutating advancement restamps or the review is run again at the new diff boundary. Host tools should not hand-edit skill verification files to add digest fields.
