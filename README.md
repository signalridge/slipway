...
- `artifacts/changes/`: governed change bundles. `artifacts/changes/<slug>/change.yaml` and top-level Markdown artifacts are project records. Active records are runtime authority; archived records are written in a Git-safe form with archive-local artifact paths.
