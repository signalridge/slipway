...
func TestNextGovernedBlocksWithoutTaskEvidenceForWaveRunSummaryDuringImplementation(t *testing.T) {
	...
	assert.Contains(t, missingEvidenceBlocker, "record_command=slipway evidence task --task-id <task_id> --verdict <verdict> --evidence-ref <ref> [--changed-file <path> ...] --json")
	assert.Contains(t, missingEvidenceBlocker, "host_fields=task_id,verdict,evidence_ref,changed_files,no_op_justification,blockers,session_id")
...
func assertSingleRunSummaryMissingTaskEvidenceBlocker(t *testing.T, surface string, blockers []model.ReasonCode, slug string) {
	...
	assert.Contains(t, missingEvidenceBlocker, "record_command=slipway evidence task --task-id <task_id> --verdict <verdict> --evidence-ref <ref> [--changed-file <path> ...] --json", surface)
	assert.Contains(t, missingEvidenceBlocker, "host_fields=task_id,verdict,evidence_ref,changed_files,no_op_justification,blockers,session_id", surface)
