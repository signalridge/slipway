...
func TestBuildNextContextFallsBackToProjectRootWithoutWorktreeBinding(t *testing.T) {
	...
	var view nextView
	loaded, _, err := buildNextContextByModeWithReadContext(newStateReadContext(root), &view, changeRef{Slug: change.Slug})
...
func TestBuildNextContextLeavesGateStatusToReadinessEvaluation(t *testing.T) {
	...
	var view nextView
	loaded, _, err := buildNextContextByModeWithReadContext(newStateReadContext(root), &view, changeRef{Slug: slug})
	require.NoError(t, err)
	require.NotNil(t, loaded)
