...
AI 编程宿主很快，但一段长时间的自主会话可能偏离你真正要求的任务、跳过它自称已经执行的验证，或者在仓库尚未体现结果时就宣称完成。
...
这就是整个 Run 的形态：一个有界 Action、一份剩余预算，以及宿主每个分支都对应的、可直接执行的下一条命令——包括永远无需理由的 skip。Action 由 CLI 选定并记录，它不会自行修改代码。加 `--json` 可得到机器可读形式。
