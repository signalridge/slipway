...
AI 编程工具很快，但一段长时间的自主会话可能偏离你真正要求的任务、声称已验证但实际并未执行，或者在仓库尚未体现结果时就宣称完成。

Slipway 是一款由用户主动调用的 辅助自动化工具。它为 AI 编程工具提供一套小而可恢复的工作流，同时把决策权和控制权留给用户。
...
> `run`、`status`、`stop`，以及 generated adapter 调用的 `protocol` 命令组的版本。
> 包管理渠道可能落后于仓库；在包含这套接口的 tag 发布之前，请从当前 checkout 构建。

## 为什么使用 Slipway？

- **主动启动：** 不会在后台或普通对话中自动触发。用户针对具体任务调用生成的能力或 CLI。
...
使用 [`go.mod`](go.mod) 声明的 Go 版本构建 Slipway，然后安装 AI 编程工具适配器：
...
在 Claude 中主动调用生成的 `slipway-run` skill，并描述一个任务。其他宿主可将 `claude` 替换为：
...
```bash
./slipway run -- "为报表增加 CSV 导出"
```

```text
Run 012a3865-18d9-4a4d-9c68-fec5cc77ae78 started.
State: active
Goal: 为报表增加 CSV 导出
Budget remaining: 7
Current action: orient (d9fe8529-3c39-4096-956a-79f42da0aea7)
Next choices:
- submit-outcome-file: requires outcome_file (path via --outcome-file)
- submit-outcome-stdin: slipway protocol submit --run 012a3865-… --action d9fe8529-… --root /path/to/reports --outcome-stdin
- skip-action: slipway protocol skip --run 012a3865-… --action d9fe8529-… --root /path/to/reports
```

这就是整个 Run 的形态：一个范围明确的 Action、一份剩余预算，以及宿主每个分支都对应的、可直接执行的下一条命令——包括永远无需理由的 skip。Action 由 CLI 选定并记录，它不会自行修改代码。加 `--json` 可得到机器可读形式。

完整交互见[入门指南](docs/zh/start-here.md)，集成细节见[机器协议](docs/zh/reference/machine-protocol.md)。
...
| GitHub Change Issue | 任务需要可审阅、按 revision 固定的需求来源。 | 稳定的 Issue 身份、范围明确的章节目录，以及按 digest 保存的已接受章节。 |
...
Run 需要输入时，生成的宿主会给出精确的决策、来源选择、环境问题或破坏性范围。只有对应的明确响应后才继续。`stop` 保留恢复数据；删除 Run 目录会移除本地恢复能力，但不是安全擦除。
...
- [从这里开始](docs/zh/start-here.md)
- [安装](docs/zh/installation.md)
- [从想法到 Run 的工作流](docs/zh/guides/idea-to-run-workflow.md)
