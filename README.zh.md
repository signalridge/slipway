...
从官方、基于发布版本的渠道安装 Slipway，然后为你实际使用的 AI 工具初始化仓库适配器。
...
| Linux | 使用[安装](docs/installation.md#linux)中的 `.deb`、`.rpm`、`.apk`、`tar.gz`、AUR 或容器镜像路径。 |
| Go 回退方案 | `go install github.com/signalridge/slipway@latest` |
...
如果要让 AI 编码工具代为安装 Slipway，让它遵循
[AI 工具安装提示词](docs/installation.md#ai-tool-installation-prompt)。
