...
- `internal/engine/wave/wave.go`: harden same-wave static target conflict detection for normalized path aliases, parent/child target overlaps, and case-only aliases so the default-parallel safety guarantee is explicit.
...
- Malformed, stale, conflicting, or unknown-wave `dispatch_mode` verification references are ignored as advisory evidence rather than blocking execution sync.
