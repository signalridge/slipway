...
## Selected Approach
Option 1. It satisfies the Approved Summary (explicit signal in `wave-plan.yaml` and `next --json`, dispatch-mode evidence, and a `parallelization` off-switch) with the smallest clean, host-driven design. The `parallel` flag is computed in `MaterializeWavePlan` from the wave's task count (safe because `validateWaveStaticConflicts` guarantees same-wave tasks are file-disjoint and dependency-free, and is hardened here for slash-normalized aliases, parent/child target overlaps, and case-only aliases) and is deliberately excluded from the tasks-plan hash inputs so freshness is unaffected. The generated skill is rewritten so concurrent dispatch of a multi-task wave is the default; a host that genuinely cannot run concurrent executors records a structured wave-orchestration verification reference such as `dispatch_mode:wave=1:degraded_sequential`, which wave sync recovers into `WaveRun.dispatch_mode` after that wave has started. Pending waves with no task evidence leave `dispatch_mode` empty. Malformed, conflicting, unknown-wave, no-longer-parallel, or not-yet-started dispatch references are treated as advisory and ignored rather than blocking execution sync. The engine signals and records but never executes work itself — Slipway stays host-driven, not a harness.
...
- Data flow unchanged otherwise: `tasks.md` → slash-normalized `target_files` parsing → `wave.PlanWaves` (with hardened same-wave static conflict checks) → `MaterializeWavePlan` (now stamps `Parallel`) → `wave-plan.yaml` → `next --json` → generated `wave-orchestration` skill → host fan-out → wave-orchestration verification references → `WaveRun` evidence (now records `dispatch_mode` for started waves).
...
- **Host without concurrent executors (low):** must degrade to sequential and record `degraded_sequential` after the wave starts; fail-open — missing, malformed, stale, or not-yet-started advisory dispatch evidence never blocks `done`.
- **Static target aliasing under default parallelism (medium):** slash/backslash path aliases, parent/child target declarations, and case-only aliases could otherwise bypass the same-wave file-disjoint guarantee. Mitigation: harden public-path normalization plus `validateWaveStaticConflicts`, and cover those cases with wave-planning, materialization, evidence, and scope-contract tests (REQ-007).
