...
Slipway also projects an `engine_boundary` object in host-facing JSON
directives. This is generated by the engine and is not accepted in
`.slipway.yaml`. It tells the host whether the slot is read-only and which
mutation policy applies. `plan_audit`, `review`, and `verify` always receive a
read-only `engine_boundary` (`read_only: true`, `mutation_policy: deny`) even
when no `subagents` config exists, so their safety boundary does not depend on
user-authored routing.
...
| --- | --- | --- |
| `default` | inherited by other slots | Shared fallback. If `subagents` is absent, read-only slots still emit native boundary-only directives. |
