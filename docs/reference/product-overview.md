...
Hosts attest GitHub fetches; Issue content remains untrusted. Change snapshots and revisions are pinned, amendments require an explicit current candidate choice, and destructive authority is one-shot and exact-scope. Publication uses approved operation/item UUID markers and reconciliation because GitHub does not provide exactly-once Issue creation or body CAS. Review reports findings without editing or opening a repair loop.

The append-only journal in the Git common directory is recovery authority. It can contain accepted Requirements, goals, answers, and command summaries, so it must be treated as sensitive local data. Slipway minimizes and redacts recognized credentials but does not promise a secret-free journal. Deleting a run directory removes recovery capability, not backups or securely erased bytes.
