...
Slipway 为 AI 编程工具提供一套轻量、由用户控制的工作流。先从你要完成的任务开始；只有在集成或维护 Slipway 时，才需要阅读机器协议和架构页面。
...
- [从想法到 Run 的工作流](guides/idea-to-run-workflow.md)——让粗略想法、Objective、Change 或已有 Run 沿着 Slipway 由用户掌控的功能边界走最短路径。
...
英文、中文、日文三套页面描述同一产品。精确的机器字段形状位于语言无关的 JSON Schema 中；任何单一语言的翻译均不构成独立的产品契约。
