...
通常应当从发布的 release 制品或基于 release 的包管理渠道来安装 Slipway。面向开发者的安装方式，例如 `go install`、Nix 和本地源码构建，则适用于以下场景：你需要尚未发布的版本、某个还没有打包的平台路径，或一个可复现的开发环境。

把 `vX.Y.Z` 替换成你想要的 release 标签。对于尚未发布的工作内容，请使用本地 checkout 构建方式。

## 官方来源

只使用 Slipway 项目自己维护的官方 release 来源：`signalridge/slipway` 下的 GitHub Releases、`ghcr.io/signalridge/slipway` 的容器镜像、来自 `signalridge/tap` 的 Homebrew Cask 条目、来自 `signalridge/scoop-bucket` 的 Scoop manifest，以及该渠道已发布时的 AUR `slipway-bin`。如果某个 AI 工具在其他 registry 中发现了同名包，请先停下来核实其归属，再决定是否安装。
...
1. 优先选择适配你平台的官方 release 归档包，或基于 release 的包管理渠道。
2. 当你想运行 Slipway 而不在主机上安装二进制时，使用容器镜像。
3. 当没有可用的 release 包，或你明确想要一个由 Go 管理的二进制时，使用 `go install`。
4. 进行开发和处理尚未发布的改动时，使用 Nix 或本地源码构建。

## Release 安装矩阵

| 平台 | Release 制品 | 包管理渠道 | 其他方式 |
| --- | --- | --- | --- |
...
release 工作流完成后，GoReleaser 还会发布 `checksums.txt`、归档包的 SBOM、校验和签名以及容器签名。包管理渠道使用可选的发布凭据；如果某个 release 没有对应渠道，请先选用直接的 release 归档包，再考虑回退到 `go install`、Nix 或本地 checkout 方式。

## 直接使用 Release 归档包

当你想拿到已发布的二进制而不经过包管理器时，使用直接的 release 归档包。下面各平台小节给出了 macOS、Linux 和 Windows 的命令。

## 包管理器

当某个 release 已发布对应渠道时，使用基于 release 的包管理器：
...
在 Windows arm64 上，当存在对应的 release 资产时，用 `arm64` 替换 `amd64`。
...
当你的环境要求做制品完整性校验时，连同资产一起下载 release 的校验和文件，并在安装前完成校验：
...
1. 适配本机操作系统和架构的、由 Slipway 项目（`signalridge`）维护的官方 release 制品或基于 release 的包管理渠道。如果缺少对应制品，不要回退到其他无关 registry 的同名包——继续下一步。
2. **macOS：** 如果有 `brew` 且 `signalridge/tap` cask 已发布，运行 `brew install --cask signalridge/tap/slipway`。否则使用对应的 `darwin_amd64` 或 `darwin_arm64` release 归档包。
3. **Linux：** 选用对应的 `linux_amd64` 或 `linux_arm64` release 归档包，或在对应渠道可用时选用 `.deb`、`.rpm`、`.apk`、AUR `slipway-bin` 或 `ghcr.io/signalridge/slipway` 容器镜像。
4. **Windows：** 如果已配置 Scoop（`signalridge/scoop-bucket`）就用它。否则使用对应的 `windows_amd64` 或 `windows_arm64` release zip。
5. 如果没有可用的基于 release 的渠道，但已安装 Go，运行 `go install github.com/signalridge/slipway@latest`。
...
- 用户需要知道的任何未决后续事项（例如本平台缺少对应 release，或 `slipway init` 的某些选择仍需人来拍板）。
...
生成的 skill 辅助命令通过 `slipway tool ...` 运行，而不是生成的脚本载荷。手动辅助命令可能仍需要明确的已认证后端或领域工具，例如用于 GitHub 辅助的 `gh`，或用于 Go 测试污染追踪的 `go`，当这些不可用时会 fail closed 并给出补救建议。
