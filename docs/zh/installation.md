...
"# Slipway local state (managed)" 块（忽略 bundle 本地的 `events/`、`verification/`、旧版的逐变更 `evidence/` 以及 `.worktrees/` 路径），同时创建仓库本地的 `.git/slipway/` 运行时区域。运行时的任务证据通过 `slipway evidence task` 记录到 `.git/slipway/runtime/changes/<slug>/evidence/` 下。除非你传入 `--tools`，否则它不会生成任何 AI 工具的接入面：
