...
本页说明证据收集，不是 release verdict 或 CLI gate。[中文产品契约](product-contract.md)定义全部 35 场景；[可执行 acceptance matrix](../../../tests/acceptance/README.md)记录当前 artifacts 与真实缺口。
...
CI matrix 会在 `windows-latest` 构建 `slipway.exe`，并分别执行 native PowerShell 与 `cmd.exe` 资产。Workflow 只是 W collector，不是预先存在的证据；只有同一次已完成运行中两者都成功才记录 W，本次本地修改不宣称已取得该运行。R gate 使用 `tests/acceptance/` 下的 stdlib link/release-artifact checker，覆盖 built-site route、archive LICENSE bytes、Scoop、AUR 与 package paths。
