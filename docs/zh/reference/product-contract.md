...
Change 是唯一 issue-backed primary source；一个 Change 有一个连贯、可观察的结果，可独立 merge/交付、验证、回滚，完成后仓库仍安全有意义，且大致适合一个 fresh Agent context。若两项可独立交付就拆开；UI/API/存储/测试可以组成同一 vertical slice。完整有效 Requirements 必须由 Change 自己的 manifest-addressed chapters 提供，不运行时继承 parent，也不扫描普通讨论 comments；讨论决定只有被发布成 replacement chapter 并进入新 manifest 后才属于下一 snapshot。
...
标题和标签：`[Change] <independently deliverable outcome>`、恰好 `level:change`、恰好一个 `kind:*`，可选 `ready-for-agent`。Marker 后是唯一严格 `slipway-manifest` JSON fence；manifest 有序数组把 chapter key/role/title 显式绑定到 GitHub comment node ID 和完整 body digest。每条 chapter comment 以匹配 key 的 `<!-- slipway-section:v1 key=KEY -->` 开始。`change/v2` profile 至少包含 `outcome|requirements|acceptance_examples|constraints|non_goals` 五类 role，但每类可拆成多个独立可寻址章节。

Parser 不从 comment 顺序、时间或 marker-like 普通评论发现 authority，不对 payload 做模型总结或 Unicode normalization。Accepted chapter 不原地编辑；修改创建 replacement comment，并在最后一步提交新 manifest。关系使用 native parent/sub-issue 与 blocked-by，普通链接只作无法建立原生关系时的 fallback。小 bug/refactor 是 Change；需要多个交付的大工作是 Objective 加 Changes；research 的交付是有证据结论，代码变化另建 Change。纯重构写 preserved behavior、可测内部 outcome 和 validation，不伪造外部变化。
...
1. 调查仓库、现有 Issues、关系、current revisions 和 accepted manifest head。
2. 生成 operation UUID、稳定 item UUID、目标 repository、chapter body digests、预期 section order/roles、exact labels/relations 和 expected parent requirements revision。
3. 展示完整 chapter drafts 与第一阶段全部外写，获得创建**非权威草稿资源**的当前确认。新 Change 先建带 reconciliation markers、但不带 `change/v2` marker 的 draft Issue shell；amendment 保持 accepted body 不动。
4. 创建 replacement chapter comments，逐条 refetch 验证 node/database IDs、body 与 visibility。未被 manifest 引用的 comments 永远只是 drafts。
5. 远端 IDs 已知后构造并展示 exact final manifest，获得更新 Issue body 这一 commit 的第二次当前确认。
6. Commit 前立即 refetch accepted head；parent revision drift 时停止并重新 preview，不做 last-writer-wins。
7. 最后写入 `change/v2` marker + manifest；reconciliation UUID markers 只能放在 manifest fence 之后，不能破坏 source parser grammar。
8. 写后 readback body digest、manifest、comments、labels、关系、node identity 与 URL，并对账完整 accepted graph。
9. timeout-after-success、partial relation failure、duplicates、index delay 或歧义时，通过 paginated non-search Issue API 按 marker 对账，不盲重试。
10. 每个 item/label/relation 报 `created|matched|failed|ambiguous`。零匹配要重新 preview/确认；一个匹配可收敛；多个匹配暂停，不自动关闭 duplicate、删除 abandoned drafts 或 rollback partial success。
11. Receipt 只含 IDs、digests、targets、expected revisions、observed URL/status，不保存正文，也不是工作权威；receipt 丢失后必须重新 preview/确认。
12. Label 创建本身也属于 plan。工具/认证不可用时报告环境不可用，不创建本地 Issue authority。
...
slipway _machine resume RUN (--source-file FILE | --use-pinned-source | --source-choice pinned|adopt --candidate ID) [--budget N]
...
Raw envelope 是最大 16 MiB 的严格 JSON：`source_version:2`、`provider:"github"`、`host:"github.com"`、repository/issue node IDs、number、canonical URL、timestamps、title、body、labels、可选 parent identity，以及 **仅且全部**由 body manifest 引用的 comments。Unknown/duplicate keys、错误类型、invalid UTF-8、BOM、控制字符和 trailing data 拒绝。文件以 no-follow 一次打开的普通文件读取并立即关闭。Host 是 trusted fetch attester，CLI 只验证内部一致性；token、Project、attachments、links 和普通讨论 comments 不进入 envelope。

Issue body 第一非空行精确为 `<!-- slipway-level: change/v2 -->`，随后是唯一严格 `slipway-manifest` JSON fence。Manifest array 是章节顺序和唯一 HEAD，最多 64 项；每项把稳定 key/role/title 绑定到 GitHub comment GraphQL node ID、REST database ID hint 和完整 comment body digest。CLI 不扫描评论、不使用显示顺序或时间戳发现 authority。每条被引用 comment 第一非空行必须精确为 `<!-- slipway-section:v1 key=KEY -->`，其后 payload 只做 CRLF/CR→LF，不 Unicode normalize、不模型重写。单章最多 256 KiB，总 payload 最多 4 MiB。缺失、额外、重复、minimized、被原地编辑、marker 错误或 hash 不匹配均 fail closed。
...
`contract_version:2`；未知版本 fail closed。上限：source envelope 16 MiB、manifest 256 KiB、section 256 KiB、bundle 4 MiB、Outcome 1 MiB、单 journal record 4 MiB、context 128 KiB、suggestion 最多 1、brief 8 KiB、Action 编码 256 KiB。超限在持久化/签发前返回结构化错误。

Action kind 只允许 `orient|clarify|implement|review|summarize`。Issue-bound Action 带 source/manifest/requirements revisions、有序 section catalog 和完整 structured `_machine material` reader，但绝不复制 Markdown；ad-hoc 同时省略 source/requirements，不能发 null。Reader 必须绑定当前 root/run/action，section 是 manifest-order enum，返回一个经 digest、byte count 与 section revision 验证的 `action_material` JSON。只有 current non-void Action 可读取；completed、replaced、stopped 或其他 stale Action 均拒绝。Context 只投影 active decisions 与非 void Outcome summary/known issues，按 current decision、其他 active decisions、最近 Outcome、其余 Outcome 优先；LF/UTF-8 确定性截断带 original bytes + SHA-256 marker，并记录 per-class omission count。相同 journal replay byte-identical；Action 大小使用与 stdout 相同的 non-HTML-escaping encoder 度量。
...
```text
orient/clarify/implement
  ├─ 新 Git revision 且 review enabled → review（安全优先，丢弃 pending suggestion）
  ├─ 无 review override 且有 suggestion → suggested Action
  └─ 其他 → summarize
review（任意合法结果） → summarize → ended
```

事实先调查；dependent decision 必须是下一 Clarify suggestion/pause，不能藏在 prose。每个 Action 可无理由 skip。Orient、Clarify、Implement 完成或 skip 后都独立观察 Git；相对上次 observation 的新 revision 是安全侧 Review 信号，优先于并清除该 Outcome 的 pending suggestion，避免 Orient/Clarify 通过 suggestion 绕过 Review；停止/恢复后产生的第二个 revision 会获得新的 Review，而 snapshot 未变不会循环。Failed/missing activity、finding、dirty worktree 和 Issue status 不控制 progression。Finding 不自动建修复 Change 或 loop。Run start 固定 canonical workspace identity、HEAD、index bytes、porcelain v2 status 和 pre-existing dirty/untracked per-path digest/状态。Observed-since-start diff 不是因果归因；用户、其他 Run 或工具可能贡献，报告必须保留 attribution uncertainty。Host 的 applied/not_needed/unable 与 start-to-current diff 不一致只记 observation/report discrepancy，不指控。
