...
每个 JSON success/error 都是顶层 `contract_version:1` object。Install/uninstall 固定为 `{contract_version,hosts,written,removed,preserved,warnings}` 且数组不省略；list 为 `{contract_version,hosts:[...]}`；无 ID status 为 `{contract_version,runs:[...]}`，空值也是 `{"contract_version":1,"runs":[]}`。单 Run status 保持 flat Run projection，顶层必须有 `contract_version` 和 fresh `next`。Doctor 为 `{contract_version,checks:[{code,status,host_id,name,detail}]}`，所有 object 在规范 schema 中均 `additionalProperties:false`。Repository/adapter code 为 `repository_ok`、`adapter_manifest_unreadable`、`adapter_not_detected`、`adapter_not_installed`、`adapter_refresh_required`、`adapter_modified`、`adapter_healthy`。
