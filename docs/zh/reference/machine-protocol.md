...
Manifest revision 不变的 refresh（相同、projection-only 或其他 non-material drift）会原子 void 旧 Action/queue/authorization 并 fresh Orient。任何新 manifest revision（包括 content-identical replacement）或 invalid body 会持久化 path-free current candidate、暂停为 `decision_required`，且不应用该调用的 budget，响应明确返回 `budget_applied:false`。选择 `pinned` 会保留已接受的 manifest、Requirements 与 section 内容，同时应用 candidate 的同一 Issue repository/number/canonical-URL/alias/parent projection；`adopt` 才安装 valid candidate snapshot。只有 adopt 改变 `requirements_revision` 时，旧 revision 派生 answer 才从 active context 移除并保留历史；manifest-only replacement 会保持 answer active。相同 `(candidate_id,choice)` 重试不产生事件或新 Action。显式 budget 必须为 1..1000；省略时保留正余额，耗尽时恢复为 `max(initial,3)`，随后 Orient 消耗一条。状态输出安全暴露 `pinned_source`、`source_candidate`、`resume_operation` 与 `budget_applied`，不会暴露 source-file 路径。
...
Requirements payload 是独立、不截断的本地 material，绝不复制进 context。Reader 会验证 digest、byte count 与 section revision，且仅 current non-void Action 可读；completed、replaced、stopped 或其他 stale Action 均拒绝。Context 只含 active confirmed decisions 与 non-void Outcome projection；优先级固定为最新 active decision、其余 active decisions（新到旧）、最新 Outcome summary 及其 known issues、其余 Outcomes（新到旧）。新 decision 只有显式设置 `pause.supersedes_answer_action_id` 并在用户回答后才停用该指定旧 answer；skip 不停用它，也不会推断其他 supersession。旧 requirements revision decision 保留在历史但排除；destructive confirmation attestation 不是产品 decision，只有独立 decline-or-feedback 分支的非空文本可成为 active feedback。每个 candidate 将 CRLF/CR 归一为 LF 并验证 UTF-8；按 class 内时间正序渲染，空间不足时只在 code-point 边界截断并追加 `...[truncated original_bytes=N sha256=HEX]`，每类遗漏用稳定 `[omitted CLASS: N]`。相同 journal replay 产生 byte-identical context，永不超过 128 KiB。
...
每个 JSON success/error 都是顶层 `contract_version:2` object。Install/uninstall 固定为 `{contract_version,hosts,written,removed,preserved,warnings}` 且数组不省略；list 为 `{contract_version,hosts:[...]}`；无 ID status 为 `{contract_version,runs:[...]}`，空值也是 `{"contract_version":2,"runs":[]}`。单 Run status 保持 flat Run projection，顶层必须有 `contract_version` 和 fresh `next`。Doctor 为 `{contract_version,checks:[...]}`；每条 check 都有 `{code,status,host_id,name,detail}`，`runstore_durability_full|runstore_durability_limited` 还带 `durability:{level,file_sync,directory_sync,limitation?}`。所有 object 在规范 schema 中均 `additionalProperties:false`。Repository/adapter code 为 `repository_ok`、`adapter_manifest_unreadable`、`adapter_not_detected`、`adapter_not_installed`、`adapter_refresh_required`、`adapter_modified`、`adapter_healthy`。
