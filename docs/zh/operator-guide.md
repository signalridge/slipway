...
| `.git/slipway/runtime/changes/<slug>/evidence/**` | 供 wave 执行和新鲜度诊断使用的运行时任务证据。 | Git 内部的本地运行时状态。 |
| `.git/slipway/runtime/changes/<slug>/handoff.md` | 面向活动变更的全新 AI 会话的可选辅助续接说明。它不是生命周期权威源、受治理的证据、新鲜度输入，也不是关卡。 | Git 内部的本地运行时状态。 |
...
`.git/slipway/runtime/changes/<slug>/handoff.md`。不要从 handoff 文本推导生命周期状态、技能选择或新鲜度。
`slipway init`、`slipway new` 和 `slipway codebase-map` 会幂等地维护 Slipway 本地状态的 `.gitignore` 块。

## Worktree

受治理的工作可以绑定到 `.worktrees/<slug>` 下的专用 worktree。使用持有当前受治理 diff 的那个 worktree：
...
不要仅凭 `main...HEAD` 判断就绪状态。把分支比较与直接的 worktree 状态和 diff 检查搭配使用。
...
为了让这种过滤可见、而不是要从 `git diff`
的不一致中推断，被豁免的文件会在 `slipway validate --json`、`slipway status --json` 和 `slipway review --json` 暴露的
`scope_contract.exempt_context_files` 字段中披露出来。
执行 `slipway done` 后，Git 可安全跟踪的归档记录会留在其所属 worktree 中；在移除该 worktree 之前先提交或合并它们。
当一个绑定到 worktree 的变更仍有未提交的源码或非活动的治理改动时，`done --json` 仍会归档，并返回一个非阻塞的
...
`unrepaired_drift` 列出仍需运维人员处理的漂移，并附带目标、原因和下一步动作。不要手工编辑新鲜度字段或时间戳；应改为重新生成所指名的证据，或对源 artifact 做一次同意图的变更修订。
...
执行新鲜度诊断是结构性的，而非基于哈希。当前执行摘要会记录任务新鲜度输入，例如 `change_id`、
`run_summary_version`、`task_id` 和 `guardrail_domain`；仅含哈希的旧式摘要会被视为陈旧，必须重新生成。

`next --json --diagnostics`、`run --json --diagnostics`、`validate --json` 和
`status --json` 会暴露新鲜度失败，包含陈旧的源/证据配对、首个陈旧成因、下游证据链、期望/当前的任务输入值、权威的 bundle 与运行时路径，以及一个安全的下一步动作。
缺失任务证据的阻塞项会包含运行时任务证据目录、
`record_command=slipway evidence task --result-file <path> --json`，以及紧凑的执行结果 schema：
`task_id,verdict,evidence_ref,changed_files,blockers,session_id`。
重复使用 `--result-file` 可原子地导入多个任务结果文件；Slipway 会对整批做预检，只要有任意文件无效或与另一个任务 ID 重复，就不写入任何任务证据。
...
1. 确认 `go run . validate --json` 报告相关的活动变更关卡均已批准。这是归档前的新鲜度/就绪关卡，并不保证同一归档 bundle 在 `done` 之后还能重新验证。
2. 确认任务证据对当前 run version 是新鲜的。
3. 确认 `git diff --check`。
4. 只暂存预期内的文件。
5. 确认 `git diff --cached --check`。
6. 当变更达到 done-ready 时运行 `slipway done --json`。活动变更 bundle 不需要在 `done` 之前提交。
