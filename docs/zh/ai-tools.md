...
Pi 把命令提示词存储在 `.pi/prompts/`，生成的 skill 存放在 `.pi/skills/`。Slipway 还会合并 `.pi/settings.json`，使 `enableSkillCommands` 为 `true`、`./skills` 列入 `skills`、`./prompts` 列入 `prompts`。
...
Claude 会注册这两个 hook。Qwen 只注册 `SessionStart`。这些通过 settings 注册的 hook 不会生成启动脚本文件；命令会在 `PATH` 上解析 `slipway` 二进制，hook 行为存在于该二进制中。Pi 的 settings 只用于注册 skill 和提示词，不涉及 hook 设置。
