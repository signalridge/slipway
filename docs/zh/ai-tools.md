...
| `codex` | `.codex/skills/slipway-*/SKILL.md` | `.codex/skills/slipway-*/SKILL.md` | `$slipway-<command>`（或 `/skills`） |
...
| `kiro` | `.kiro/skills/slipway-*/SKILL.md` | `.kiro/skills/slipway-<command>/SKILL.md` | `@slipway:<command>` 或宿主 skill 选择器 |
...
| `qwen` | `.qwen/skills/slipway-*/SKILL.md` | `.qwen/skills/slipway-<command>/SKILL.md` | `/slipway-<command>` 或宿主 skill 选择器 |
...
Slipway 通过它生成的哨兵文件来检测适配器，而不是仅凭一个裸的 `.claude`、`.codex`、`.cursor`、`.opencode`、`.pi`、`.qwen`、`.kiro`、`.windsurf` 或 `.kilocode` 目录。所有权清单会在刷新时保护生成的文件；只有哨兵、缺少清单的遗留适配器可以被纳入清单跟踪，而缺少哨兵的路径冲突则保持失败即停，除非现有内容已经与生成的输出一致。Copilot 把这部分受管状态保存在 `.github/copilot/slipway` 下，而不是把整个共享的 `.github` 目录树都当作适配器所有。刷新会移除 Slipway 所拥有的遗留 shell hook 启动脚本和已废弃的 `bash "<hook>.sh"` hook 配置项，同时保留生成文件旁边用户自有的 hook、提示词、workflow 和 skill。
...
生成的 hook 除了 `slipway` 二进制外没有其他依赖。手动辅助命令可以使用显式的、已认证的后端或领域工具：GitHub 相关辅助命令优先使用 `gh`，在 `gh` 不可用或报告需要认证的错误时回退到 token API；两种后端都不存在时则失败即停。
...
- `evidence`（`$slipway-evidence`；wave-orchestration 宿主通过 `slipway evidence task ...` 记录任务证据）

辅助命令:

- `tool` 仅限 CLI。没有 `$slipway-tool`，也没有生成的宿主提示词包装；生成的 skill 会直接调用 `slipway tool <helper>`。
...
Copilot 把命令提示词存储在 `.github/prompts/` 下，使用 `.prompt.md` 扩展名，生成的 skill 存放在 `.github/skills/`。它的哨兵和所有权清单位于 `.github/copilot/slipway` 下。

Pi 把命令提示词存储在 `.pi/prompts/`，生成的 skill 存放在 `.pi/skills/`。Slipway 还会合并 `.pi/settings.json`，使 `enableSkillCommands` 为 `true`、`./skills` 列入 `skills`、`./prompts` 列入 `prompts`。此外，Slipway 会生成 `.pi/extensions/slipway-hooks.ts`，把 `session-start` hook 桥接到 pi 的 `session_start` / `before_agent_start` 扩展事件（pi 会自动发现 `.pi/extensions/`，但只有在项目被信任后才加载项目本地扩展）。`context-pressure` hook 不会桥接到 pi：它从 stdin 读取指标，而 pi 的扩展 `exec` 没有 stdin 通道，并且 pi 会原生管理上下文压力。

Qwen 和 Kiro 把命令以生成的命令 skill 形式暴露，而不是单独的提示词文件。Qwen 会为会话启动 hook 写入 `.qwen/settings.json`。Kiro 的命令 skill 使用 `@slipway:<command>`。
...
Claude 会注册这两个 hook。Qwen 只注册 `SessionStart`。这些通过 settings 注册的 hook 不会生成启动脚本文件；命令会在 `PATH` 上解析 `slipway` 二进制，hook 行为存在于该二进制中。Pi 的 `settings.json` 只用于注册 skill 和提示词；它的 `session-start` hook 改为通过生成的 `.pi/extensions/` 模块来桥接。
