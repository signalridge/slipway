...
Pi 把命令提示词存储在 `.pi/prompts/`，生成的 skill 存放在 `.pi/skills/`。Slipway 还会合并 `.pi/settings.json`，使 `enableSkillCommands` 为 `true`、`./skills` 列入 `skills`、`./prompts` 列入 `prompts`。此外，Slipway 会生成 `.pi/extensions/slipway-hooks.ts`，把 `session-start` hook 桥接到 pi 的 `session_start` / `before_agent_start` 扩展事件（pi 会自动发现 `.pi/extensions/`，但只有在项目被信任后才加载项目本地扩展）。
...
Claude 和 Qwen 只注册 `SessionStart`。这些通过 settings 注册的 hook 不会生成启动脚本文件；命令会在 `PATH` 上解析 `slipway` 二进制，hook 行为存在于该二进制中。Pi 的 `settings.json` 只用于注册 skill 和提示词；它的 `session-start` hook 改为通过生成的 `.pi/extensions/` 模块来桥接。
