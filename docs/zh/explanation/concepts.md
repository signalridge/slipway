...
Run 必须显式启动。Slipway 不监听聊天，不把普通对话自动转换成工作，也不通过 ambient hook 启动。
...
Run 中的 source snapshot 不会静默变化。刷新 Change Issue 后若已接受内容不同，Slipway 会暂停，要求显式选择保留 pinned snapshot 或采用 candidate。若 amendment 基于另一条 history，则必须启动新 Run。
