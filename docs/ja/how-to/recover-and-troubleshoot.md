...
ライフサイクル権威、証跡の判定結果、タイムスタンプ、ランタイムのタスク証明を
手で編集してはいけません。
...
不整合な鮮度入力として、`validate --json` または `next --json --diagnostics` に
現れます。
...
返された修復コントラクトを、フレッシュコンテキストの修復エージェントに渡します。
...
選定レビュアーの証跡は、現在の差分、計画アーティファクト、実行サマリーの入力に
対して新鮮でなければなりません。唯一の権威ある完全スイートは、ピアが収束した後に
...
| レビューで問題が見つかった | `slipway fix --json` | フレッシュコンテキストで修復し、影響を受けたレビュアーを再実行する。 |
