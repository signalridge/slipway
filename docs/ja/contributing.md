...
このページは root の[コントリビューション](../../CONTRIBUTING.md)を補い、repository 固有の layout と documentation check を説明します。
...
並行処理、locking、journal、filesystem の変更では race suite も実行します。
...
- Host-side instruction は Go CLI guarantee ではなく host behavior として書く。
...
| 変更 | 重点 |
| --- | --- |
| CLI flag や output | Cobra help、JSON schema test、human rendering、command docs。 |
...
変更は、明示的起動、ユーザー制御、質問より先の事実確認、正直な activity 報告、read-only Review、recoverable な journal、ownership-aware generated file、network/credential を持たない core を維持する必要があります。公開 surface の変更時は、code、schema、generated capability、test、3言語の documentation を同時に更新してください。
