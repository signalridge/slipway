...
| 4. レビュー指摘の修正 | S3 レビューが対応すべき問題を検出した。 | フレッシュコンテキストの修正で対応を集約する。 |
...
6. S3 レビューを実行し、選定されたレビュアーがフレッシュなときにのみクローズする。
...
3. 返された修正コントラクトを、フレッシュコンテキストの修正エージェントに渡す。
4. 影響を受けた選定レビュアーを再実行する。
5. fix とレビューの context-origin の証跡がフレッシュになってから、レビューを継続する。

完了の条件:

- 修正が、選定された指摘を根本原因から解消している。
- 修正後にレビュー証跡がリフレッシュされた。
...
- フレッシュさが、所有するコマンドまたはスキルによって再生成された。
