...
Host は structured **Outcome** で回答します。Slipway は Outcome を validate/record し、Git を独立観測して次の Action を選びます。Reported activity は成功の証明ではなく、observed diff は原因となった process の証明ではありません。
...
Repository の owner が personal account か Organization かは、この model に影響しません。[GitHub Issue workflow](../guides/github-issues.md)を参照してください。

## Budget と pause

Action budget は CLI が issue できる Action 数を制限するだけで、time estimate や quality score ではありません。Run は human decision、source choice、environment unavailable、exact destructive confirmation でも pause します。
...
`ended` は automatic Action queue が空であることだけを示します。Test pass、finding なし、branch protection、PR approval、release readiness を意味しません。Slipway が事実を報告し、user と repository policy が次を決めます。

## Local state

Recovery data は `<git-common-dir>/slipway/runs/` にあります。Append-only journal が transition を記録し、replaceable projection が read を補助し、accepted Issue section は content-addressed material として保存されます。[Run、復旧、プライバシー](../guides/runs-and-recovery.md)を参照してください。
