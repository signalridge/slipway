...
![Slipway の trust boundary: Issue content と working tree は untrusted data であり、shell 権限の付与、credential の開示、confirmation の回避、destructive scope の拡大はできません。AI coding host は行為を託され、すべての credential を保持します。Local CLI は厳密な JSON・サイズ・identity・digest を検証しますが credential は一切持たず、host が GitHub を誠実に fetch したことは証明できません。](../../assets/diagrams/trust-boundary.svg)
