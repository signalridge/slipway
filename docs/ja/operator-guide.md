...
ガバナンス変更が残っている場合でも、`done` はアーカイブを実行し、ブロックしない
...
`next --json --diagnostics`、`run --json --diagnostics`、`validate`、
...
`record_surface=slipway evidence task --json --help`、そして host-owned fields が含まれます。
...
1. `go run . validate` が、対象となるアクティブな変更のゲートを承認済みと
...
6. 変更が done-ready になったら `slipway done` を実行する。アクティブな変更
