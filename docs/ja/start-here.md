...
Host を離れずに rough idea、Objective、Change、既存 Run の次の段階を判断するには、生成された `slipway-workflow` capability を呼び出します。Current stage を確認し、最短の次の explicit capability または明示的な no-further-action outcome を示します。Draft が必要なら、同じ呼び出しで fact を調査し、genuine な human decision を 1 つずつ問い、self-contained な Change または planning Objective を合成します。汎用 skill catalog の routing、user-only external skill の呼び出し、Issue publish、Run start は行いません。[アイデアから Run までのワークフロー](guides/idea-to-run-workflow.md)を参照してください。
