...
| レビューエビデンス | 現在のワークトリーと一致しなければならない検証記録。 |
...
その後、示されたリカバリーコマンドに従います。`change.yaml`、検証 YAML、タスクエビデンス、ライフサイクルのタイムスタンプを手作業で編集してはいけません。エビデンスが陳腐化している場合は、それを所有するステージ、レビュアー、またはタスクエビデンスのパスを再実行し、Slipway が現在のワークトリーから現在性を再導出できるようにします。
