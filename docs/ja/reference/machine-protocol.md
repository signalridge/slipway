...
Top-level source schema は invalid refreshed head で空 comments array を許可し、CLI が無関係な議論を収集せずに drift を分類できるようにします。Embedded manifest string と semantic digest check は runtime が検証し、top-level schema 単独ではありません。
...
Repository-wide `status` は read-only の例外で、別 linked worktree の Run は `workspace_foreign` を持つ FirstEvent header stub として表示されますが、owning worktree の外で完全に replay や mutation されることはありません。
